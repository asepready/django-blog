...
class ArticleListView(ListView):
    # template_name Attributes are used to specify which template to use for rendering
    template_name = 'blog/article_index.html'

    # context_object_name The attribute is used to name the context variable (use the name in the template)
    ...
    def get_queryset_cache_key(self):
        """
        Subclass rewriting.Get queryset Cache key
        """
...
def fileupload(request):
    """
    This method needs to write the calling terminal to upload the picture, this method only provides the image bed function
    :param request:
    :return:
    """
...
def page_not_found_view(
    ...
    return render(request,
                  template_name,
                  {'message': 'Oops, the address you visited ' + url + ' It is an unknown place. Please click on the homepage to see others?',
                   'statuscode': '404'},
                  status=404)
...
def server_error_view(request, template_name='blog/error_page.html'):
    return render(request,
                  template_name,
                  {'message': 'Oops, something went wrong. I have collected the error information, and I will fix it soon. Please click on the homepage to see other things?',
                   'statuscode': '500'},
                  status=500)
